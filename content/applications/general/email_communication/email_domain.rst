...
relays the reply to the followers.

Emails from the database users to partners (customers or vendors) are sent from the email addresses
of the database users. Similarly, emails from partners to users are sent from the email addresses
of the partners. That way, the database users can recognize who sent an email relayed by Odoo.
...
To ensure that emails from the database users reach the partners, instead of being considered spam,
Odoo recommends configuring the domain name.

For the same reason, Odoo also recommends giving each database user an email address from the
configured domain, rather than a generic email address domain (gmail.com, outlook.com, etc.).
...
Check if the :abbr:`SPF (Sender Policy Framework)` record is valid with a free tool like
...
signature in the headers. The recipient's server decrypts this signature using the
:abbr:`DKIM (DomainKeys Identified Mail)` record in the database's domain name. If the signature
and the key contained in the record match, this guarantees that the message is authentic and has
...
Check if the :abbr:`DKIM (DomainKeys Identified Mail)` record is valid with a free tool like
...
that fails the :abbr:`SPF (Sender Policy Framework)` and/or :abbr:`DKIM (DomainKeys
Identified Mail)` check.
